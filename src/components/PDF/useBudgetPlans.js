...
import { getProfileCode } from '../PDF/auth';
...
export default function useBudgetPlans(objPDF) {
  ...
  const handlePays = (plan) => {
    ...
    const payment = payments.map((payment) => {
      if (payment?.id === 0) {
        ...
        const pay = plan.fraccionamiento.find(
          (element) => element.maxgiro === payment.id
        );
        if (pay === undefined) {
          return 'CLOSE';
        } else {
         // console.log(1,pay.prima)
          return pay.prima;
  ...
  const checkSelectedPay = (plan_id) => {
    if (selectedPays !== null) {
      const indexPlan = selectedPays.findIndex(
        (element) => element.plan_id === plan_id
      );
      const checkSelected = selectedPays[indexPlan].methods.some(
  ...
  const handleCheckPays = (plan) => {
    ...
    if (checkSelectedPay(plan.plan_id) === true) {
      payment = payments.map((payment) => {
        if (payment?.id === 0) {
          const indexPlan = selectedPays.findIndex(
            (element) => element.plan_id === plan.plan_id
          );
          const indexMethod = selectedPays[indexPlan].methods.findIndex(
            (element) => element.id === payment.id
          );
          const methodChecked =
            selectedPays[indexPlan].methods[indexMethod].checked;
  ...
  const handleCoberts = (plan) => {
    //console.log(cobertsDescrip)
    const coberts = cobertsDescrip.map((cobertsDescrip) => {
     // console.log(cobertsDescrip.codcobert)
      const cobert = plan.coberturas.find(
        (element) => element.codcobert === cobertsDescrip.codcobert
  ...
  const handleProperties = (plan) => {
    const properties = propertyDescrip.map((p) => { 
      ...
      const result = filteredProperties.map((description) => {
        const resultProperty = cobertsProperty.find(
          (cob) =>
            plan.codprod === cob.codprod &&
            plan.codplan === cob.codplan &&
            plan.revplan === cob.revplan &&
            cob.codcobert === description.id
        );
  ...
  const handleAges = (plan) => {
    const ages = budgetInfo.insured.map((a) =>
      plan.coberturas.findIndex((p) => p.age === a.age) === -1
        ? { age: a.age, inc: 'N' }
        : { age: a.age, inc: 'S' }
    );
    return ages;
