...
export default function ModalResumePDF(props) {
    ...
    for (const plan of plans) {
      coberts = [
        ...
        ...plan.coberturas.filter(
          (c) => c.indincluida === 'S' && c.indvisible === 'S'
        ),
      ];
    }
  ...
  function generateDistinctPay(plans) {
    ...
    const distinctPays = distinctArray(pays, 'maxgiro', 'nomplan').sort(
      (a, b) => a.id - b.id
    );
          ...
          <div className={classes.paper}>
            <GridContainer>
              ...
              >
                ...
                {nextStep && typeModal === 'MAIL' && (
                  <FormMailPDF handleClose={handleClose} objPDF={objPDF} />
                )}
              </GridItem>
            </GridContainer>
          </div>
