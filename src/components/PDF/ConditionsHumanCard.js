...
export default function ConditionHumanCard({ infoClient }) {
  ...
  useEffect(() => {
    if (infoClient) {
      const result = difDays(
        infoClient[0].DATE_CREATION,
        infoClient[0].EXPIRED_ON
      );
      setTotalDays(result);
    }
  }, []);
  ...
  return (
    <View style={styles.cardConditional} wrap={false}>
      ...
      {insuranceCompany === 'OCEANICA' ? (
        <ConditionsHumanTextOceanica diffDays={totalDays} />
      ) : (
        <ConditionsHumanTextPiramide />
      )}
    </View>
  );
}
